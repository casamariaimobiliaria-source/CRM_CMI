...
const StatCard: React.FC<StatCardProps> = ({ label, value, icon, colorClass, index }) => {
    return (
        <motion.div
            ...
            className="group relative overflow-hidden rounded-[2.5rem] bg-card/30 p-8 transition-all duration-700 hover:bg-card/40 border border-white/5 hover:border-primary/20 glass-high-fidelity"
        >
            ...
            <div className="relative z-10 space-y-4">
                <div className="flex items-center justify-between">
                    ...
                    <div className={cn(
                        "flex h-10 w-10 items-center justify-center rounded-xl bg-white/5 border border-white/5 transition-all duration-700 group-hover:scale-110 group-hover:border-primary/20 group-hover:bg-primary/5",
                        colorClass
                    )}>
                        {icon}
                    </div>
